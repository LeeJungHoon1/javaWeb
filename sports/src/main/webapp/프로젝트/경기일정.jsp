...
function num( userid,gamenum) {
	window.location.href = "좌석.jsp?id=" + userid + "&gamenum=" + gamenum;
}
...
<nav>
	...
	<li><a href="manager.jsp">관리자 페이지</a></li>
...
<tr>
		...
		<td>
		 ...
		 <% session.setAttribute("userid", userid);
               session.setAttribute("gamenum", rs.getString(1)); %>
		 <button onclick="num('<%= userid %>', <%= rs.getString(1) %>)">예매</button>
		 <%}else{ %>
			 <%=rs.getString(6)%>
			<% } %>
